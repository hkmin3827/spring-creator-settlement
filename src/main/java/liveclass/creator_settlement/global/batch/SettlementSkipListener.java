...
@Slf4j
@Component
public class SettlementSkipListener implements SkipListener<Creator, SettlementBatchItem> {

    @Override
    public void onSkipInWrite(SettlementBatchItem item, Throwable t) {
        log.error("[배치 최종 실패 !!!] 관리자 수동 후처리 필요 - creatorId: {}, yearMonth: {}, error: {}",
                item.creatorId(), item.yearMonth(), t.getMessage());
    }

    @Override
    public void onSkipInProcess(Creator item, Throwable t) {
        log.error("[배치 최종 실패 !!!] processor 처리 실패 - creatorId: {}, error: {}",
