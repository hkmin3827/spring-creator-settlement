...
**초기 설계**: PENDING 계산 결과를 Redis TTL 1시간으로 캐싱.

**제거 이유**: TTL 근거 없음. PENDING은 판매·취소가 발생할 때마다 바뀌므로 캐시 일관성 보장 불가. confirm 이후에는 SettlementLog가 존재하므로 Redis가 담당하던 역할 자체가 사라짐.
...
- **월말 배치 스케줄러 미구현**: `confirm()` API는 배치 호출을 전제로 설계되어 있으나 스케줄러 코드는 미작성. 월초에 전월 크리에이터를 일괄 confirm하는 `@Scheduled` 잡 추가 필요.
- **크리에이터/강의 등록 API 없음**: 초기 데이터는 `DataInitializer`(서버 시작 시 더미 데이터 삽입)로만 제공됩니다.
- **인증/인가 없음**: 모든 API는 인증 없이 접근 가능합니다.
...
이 프로젝트는 Claude(Anthropic)와의 페어 프로그래밍으로 개발되었습니다. 아래는 대화를 통해 결정되거나 리팩토링된 주요 사항입니다.

- **Settlement 아키텍처 재설계**: 금액 필드를 Settlement에 저장하는 초기 구조의 문제를 제기하여, Settlement(마커) + SettlementLog(스냅샷) 분리 구조로 전환했습니다.
- **Redis 제거**: PENDING 정산의 Redis 캐싱 근거가 없음을 지적하여 전체 Redis 의존성을 제거하고 실시간 계산 방식으로 전환했습니다.
